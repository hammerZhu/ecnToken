...
contract EcnTokenSaleMulti is Ownable, TokenHolder {
    ...
    uint[3] saleRates;//=[80,60,50];//pencents of ECN_PER_WEI
    uint[3] saleDelays;//=[15,30,60];
    ...
    function EcnTokenSaleMulti(address _fundingRecipient,address _ecnToken) {
        ...
        saleRates[1]=60;
        saleRates[2]=50;
        saleDelays[0]=15 days;//100 second
        saleDelays[1]=30 days;
        saleDelays[2]=60 days;

        //ecn.approve(this,MAX_TOKENS_SOLD);
    }
    ...
    function checkTokenCanSaled() constant returns (uint256){
        ...
        uint256 tokenLeftInSale = SafeMath.min256(allowToken,MAX_TOKENS_SOLD.sub(tokensSold));
        return tokenLeftInSale;
    }

    /// @dev receive wei and caculate token saled.
    /// @return tokens will be transfered.
    function receiveWei(address _buyer,uint256 _value,uint _rate) returns (uint256){
        uint256 tokenLeftInSale=checkTokenCanSaled();
        uint256 weiLeftInSale = tokenLeftInSale.div(_rate);
    ...
    function saleTokenDelay(address buyer,uint256 value,uint delayPeriod) private {
        ...
        //uint256 weiLeftInSale = restToken.div(rateBuying);
        //uint256 weiToParticipate = SafeMath.min256(value, weiLeftInSale);
    ...
    function withdrawAvailTokens() external{
        ...
        for(uint i=0;i<MAX_NUM_OF_SALEITEM;i++){
            if(saleItem[i].value>0  && now>saleItem[i].timeAvail && saleItem[i].user==msg.sender){
                ...
                if(totalTokens+saleItem[i].value<restToken){
                    totalTokens+=saleItem[i].value;
                    saleItem[i].value=0;
                }
                else
