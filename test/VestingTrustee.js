...
contract('VestingTrustee', (accounts) => {
    ...
    beforeEach(async () => {
        ...
        token = await EcnToken.new({from: granter});
    ...
    describe('construction', async () => {
        ...
        context(`with ${balance} tokens assigned to the trustee`, async () => {
            beforeEach(async () => {
                ...
                await token.transfer(trustee.address, balance,{from:granter});
            });
                ...
                context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                    ...
                    for (let i = 0; i < grant.results.length; ++i) {
                        it(`should revoke the grant and refund tokens after ${i + 1} transactions`, async () => {
                            ...
                            await token.transfer(trustee.address, grant.tokens,{from:granter});
                            ...
                            assert.approximately(trusteeBalance3, trusteeBalance2 , MAX_ERROR);
                            ...
                            assert.approximately(granterBalance2, granterBalance , MAX_ERROR);
    ...
    describe('vestedTokens', async () => {
        ...
        [
            ...
            {
                tokens: 10000000, startOffset: 0, cliffOffset: 0, endOffset: 2 * YEAR, installmentLength: 1, results: [
                    { offset: 0, vested: 0 },
                    { offset: MONTH, vested: Math.floor(10000000 / 12 / 2) },
                    { offset: 2 * MONTH, vested: Math.floor(2 * (10000000 / 12 / 2)) },
                    { offset: 0.5 * YEAR, vested: 10000000 / 4 },
                    { offset: YEAR, vested: 10000000 / 2 },
                    { offset: 2 * YEAR, vested: 10000000 },
                    { offset: 3 * YEAR, vested: 10000000 }
                ]
            },
            {
                tokens: 10000000, startOffset: 0, cliffOffset: 0, endOffset: 2 * YEAR, installmentLength: YEAR, results: [
                    { offset: 0, vested: 0 },
                    { offset: MONTH, vested: 0 },
                    { offset: 2 * MONTH, vested: 0 },
                    { offset: 0.5 * YEAR, vested: 0 },
                    { offset: YEAR, vested: 10000000 / 2 },
                    { offset: YEAR + MONTH, vested: 10000000 / 2 },
                    { offset: YEAR + 2 * MONTH, vested: 10000000 / 2 },
                    { offset: YEAR + 10 * MONTH, vested: 10000000 / 2 },
                    { offset: 2 * YEAR, vested: 10000000 },
                    { offset: 3 * YEAR, vested: 10000000 }
                ]
            },
    ...
    describe('unlockVestedTokens', async () => {
        ...
        it('should allow revoking multiple grants', async () => {
            ...
            assert.equal(trusteeBalance2, trusteeBalance);
            for (let grant of grants) {
